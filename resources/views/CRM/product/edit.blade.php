...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        ...
        <div class="card-body">
            <div id="message"></div>
            <form id="save" method="post" action="{{ url('crm/products/'. $product->_id) }}" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="stock" id="stock" class="form-control"
                                    placeholder="Enter stock quantity"value="{{ $product->stock }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="product_type" id="product_type" class="form-select">
                                    ...
                                    <option value="primary1" {{ $product->product_type == 'primary1' ? 'selected' : '' }}>Primary</option>
                                    <option value="rp" {{ $product->product_type == 'rp' ? 'selected' : '' }}>RP</option>
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="product_section" id="product_section" class="form-select">
                                    <option value="">Select Section</option>
                                    <option value="primary" {{ $product->product_section == 'primary' ? 'selected' : '' }}>Primary</option>
                                    <option value="deals" {{ $product->product_section == 'deals' ? 'selected' : '' }}>Deals</option>
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <div class="form-check form-switch">
                                    <input type="checkbox" class="form-check-input" id="is_combo" name="is_combo" value="1" {{ $product->is_combo ? 'checked' : '' }}>
                            ...
                            <div id="product_select_container" class="col-md-12 mb-3" style="display: {{ $product->is_combo ? 'block' : 'none' }};">
                                ...
                                <select name="product_ids[]" id="product_ids" class="form-select select2" data-placeholder="Select Products" multiple {{ $product->is_combo ? 'required' : '' }}>
                                    @foreach($products as $comboProduct)
                                        <option value="{{ $comboProduct->_id }}" {{ in_array($comboProduct->_id, $product->product_ids ?? []) ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="category_id" id="category_id" class="form-select" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->_id }}" {{ ($product->category_id == $category->_id) ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                        @if($category->children->count() > 0)
                                            @foreach($category->children as $child)
                                                <option value="{{ $child->_id }}" {{ ($product->category_id == $child->_id) ? 'selected' : '' }}>
                                                    &nbsp;&nbsp;&nbsp;&nbsp;{{ $child->name }}
                                                </option>
                                                @if($child->children->count() > 0)
                                                    @foreach($child->children as $grandChild)
                                                        <option value="{{ $grandChild->_id }}" {{ ($product->category_id == $grandChild->_id) ? 'selected' : '' }}>
                                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ $grandChild->name }}
                                                        </option>
                                                    @endforeach
                                                @endif
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="brand_id" id="brand_id" class="form-select select2" data-placeholder="Select Brand" required>
                                    ...
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->_id }}" {{ $product->brand_id == $brand->_id ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" name="mrp" id="mrp" class="form-control" 
                                    placeholder="Enter MRP" value="{{ $product->mrp }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" name="sale_price" id="sale_price" class="form-control" 
                                    placeholder="Enter sale price" value="{{ $product->sale_price }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" name="cgst" id="cgst" class="form-control" 
                                    placeholder="Enter GST" value="{{ $product->cgst }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" name="up" id="up" class="form-control" 
                                    placeholder="Enter unit price" value="{{ $product->up }}">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" name="sv" id="sv" class="form-control" 
                                    placeholder="Enter selling value" value="{{ $product->sv }}">
                                    ...
                                    <div class="variant-row border rounded p-2 mb-2">
                                        <div class="row">
                                            <div class="col-md-2">
                                                ...
                                                <input type="text" class="form-control form-control-sm" value="{{ $variant->sku }}" readonly>
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label">Stock</label>
                                                <input type="number" name="variants[stock][]" class="form-control form-control-sm" value="{{ $variant->stock??'' }}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Attributes</label>
                                                <div class="row">
                                                    <div class="col-6">
                                                        <input type="text" name="variants[attributes][color][]" class="form-control form-control-sm" placeholder="Color" value="{{ !empty($variant->attributes['color'])?$variant->attributes['color']:'' }}">
                                                    </div>
                                                    <div class="col-6">
                                                        <input type="text" name="variants[attributes][size][]" class="form-control form-control-sm" placeholder="Size" value="{{ !empty($variant->attributes['size'])?$variant->attributes['size']:'' }}">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-1">
