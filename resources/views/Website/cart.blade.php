    ...
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="cart-items">
                <h5 class="cart-header ">Shopping Cart ({{total_cart_count()}} Items) <a href="{{url('product')}}" class="continue">Continue Shopping</a></h5>
                @php
                	$subtotal = 0;
                	$total_mrp = 0;
                	$total_saving = 0;
        ...
        <div class="col-lg-4">
            ...
            <div class="cart-summary">
                ...
                <a href="checkout.html" class="thm-btn w-100 mb-3">Proceed to Pay ₹{{$subtotal}}</a>
                <button class="btn btn-outline-danger w-100 mb-3" onclick="clearCart()">Clear Cart</button>

                <div class="summary-box">
                    <h6 class="mb-2"><span class="text-black">Order Summary</span> ({{total_cart_count()}} Items)</h6>
...
$(document).ready(function() {
    ...
    // Clear cart function (global scope for onclick)
    ...
    function updateCartQuantity(productId, quantity) {
        ...
        $.ajax({
            ...
            success: function(response) {
                hideCartLoading();
                if (response.status) {
                    updateCartSummary(response.cart_data);
                    showSnackbar(response.msg, 'success');
                } else {
                    showSnackbar(response.msg, 'error');
